...
# -------------------------------
merged_all <- inner_join(w8w22_sel, w8m8_sel, by = "Gene_name")
...
merged_all_clean <- merged_all %>%
  filter(!is.na(W8W22_Log2FoldChange) & !is.na(W8M8_Log2FoldChange))
...
merged_all_clean <- merged_all_clean %>%
  mutate(Significant_Both = ifelse(`P-Value [W8W22]` < 0.05 & `P-Value [W8M8]` < 0.05,
                                  "YES", "NO"))

# -------------------------------
# 6) Plot 1: ALL genes scatter (no filtering)
# -------------------------------
p_all <- ggplot(merged_all_clean, aes(x = W8W22_Log2FoldChange, y = W8M8_Log2FoldChange)) +
...
print(p_all)
ggsave("Scatter_AllGenes_W8W22_vs_W8M8.png", p_all, width = 6, height = 5, dpi = 300)
...
cor_safe <- function(df) {
  ...
  cor(df$W8W22_Log2FoldChange, df$W8M8_Log2FoldChange, method = "pearson", use = "complete.obs")
}
...
# -------------------------------
p1 <- ggplot(coding, aes(x = W8W22_Log2FoldChange, y = W8M8_Log2FoldChange)) +
...
print(p1)
ggsave("Scatter_SigBoth_ProteinCoding_W8W22_vs_W8M8.png", p1, width = 6, height = 5, dpi = 300)

# -------------------------------
# 9) Plot 3: Non-coding/other (significant in both)
# -------------------------------
p2 <- ggplot(noncoding, aes(x = W8W22_Log2FoldChange, y = W8M8_Log2FoldChange)) +
...
write.csv(coding, "Merged_SigBoth_ProteinCoding_W8W22_vs_W8M8.csv", row.names = FALSE)
write.csv(noncoding, "Merged_SigBoth_NonCoding_W8W22_vs_W8M8.csv", row.names = FALSE)

# -------------------------------
# Reproducibility: save session information
# -------------------------------
writeLines(capture.output(sessionInfo()), "sessionInfo_CombinedW8W22M8.txt")
