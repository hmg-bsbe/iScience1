...
if ("gene_biotype" %in% colnames(w8l8)) {
  ...
  colnames(w8l8_sel) <- c("Gene_name", "W8L8_Log2FoldChange", "P-Value [W8L8]", "Type")
} else {
  w8l8_sel <- w8l8 %>% select(gene_name, log2FoldChange, pvalue)
  colnames(w8l8_sel) <- c("Gene_name", "W8L8_Log2FoldChange", "P-Value [W8L8]")
  w8l8_sel$Type <- NA_character_
}

w8m8_sel <- w8m8 %>% select(gene_name, log2FoldChange, pvalue)
colnames(w8m8_sel) <- c("Gene_name", "W8M8_Log2FoldChange", "P-Value [W8M8]")

# -------------------------------
# 3) Merge (ALL genes present in BOTH sheets)
#    inner_join = intersection by gene_name
# -------------------------------
merged_all <- inner_join(w8l8_sel, w8m8_sel, by = "Gene_name")
...
merged_all <- merged_all %>%
  mutate(
    ...
    W8M8_Log2FoldChange  = as.numeric(W8M8_Log2FoldChange)
  )

n_total_merged <- nrow(merged_all)

merged_all_clean <- merged_all %>%
  filter(!is.na(W8L8_Log2FoldChange) & !is.na(W8M8_Log2FoldChange))
...
cor_allgenes <- cor(merged_all_clean$W8L8_Log2FoldChange,
                    merged_all_clean$W8M8_Log2FoldChange,
                    method = "pearson",
                    use = "complete.obs")
...
merged_all_clean <- merged_all_clean %>%
  mutate(Significant_Both = ifelse(`P-Value [W8L8]` < 0.05 & `P-Value [W8M8]` < 0.05,
                                  "YES", "NO"))

# -------------------------------
# 6) Plot 1: ALL genes scatter (no filtering)
# -------------------------------
p_all <- ggplot(merged_all_clean, aes(x = W8L8_Log2FoldChange, y = W8M8_Log2FoldChange)) +
...
print(p_all)
ggsave("Scatter_AllGenes_W8L8_vs_W8M8.png", p_all, width = 6, height = 5, dpi = 300)
...
# -------------------------------
p1 <- ggplot(coding, aes(x = W8L8_Log2FoldChange, y = W8M8_Log2FoldChange)) +
...
print(p1)
ggsave("Scatter_SigBoth_ProteinCoding_W8L8_vs_W8M8.png", p1, width = 6, height = 5, dpi = 300)

# -------------------------------
# 9) Plot 3: Non-coding/other (significant in both)
# -------------------------------
p2 <- ggplot(noncoding, aes(x = W8L8_Log2FoldChange, y = W8M8_Log2FoldChange)) +
...
write.csv(coding, "Merged_SigBoth_ProteinCoding_W8L8_vs_W8M8.csv", row.names = FALSE)
write.csv(noncoding, "Merged_SigBoth_NonCoding_W8L8_vs_W8M8.csv", row.names = FALSE)

# -------------------------------
# Reproducibility: save session information
# -------------------------------
writeLines(capture.output(sessionInfo()), "sessionInfo_CombinedW8L8M8.txt")
